...
class Data():
    ...
    def create(self):
        if self.train:
            self.x, self.y = loadlocal_mnist(images_path=self.data['x_train'], labels_path=self.data['y_train'])
        elif self.test:
            self.x, self.y = loadlocal_mnist(images_path=self.data['x_test'], labels_path=self.data['y_test'])
        else:
    ...
    def get_minibatch(self):
        ...
        return (self.x[start:end,:], end-start) # returning minibatch and size

    def display(self,element):
        img = np.reshape(self.x[element,:],(28,28))
